...
This game uses other games to run, like an add-on expansion and it is an add-on expansion for all IWADs/Standalone games made in IDTECH1 and all primary standard IWADs/Standalone games made in IDTECH1 work out of the box, For any un-original content this game only uses dictionary lookups on IWADS for that content. This mod only works on UZDoom source port. The .pk3 File in the folder is the game itself but you would need all of the files in that folder for it to run and the folder itself.
...
<a href="https://github.com/Daniel-Hanrahan-Tools-and-Games/IDTECH1_Game_V._Bug_Invasion">Game Repository Page</a>
